<?php
namespace Arm\T3sixshop\Library\Pdf;
...
class Pdf {
	...
	/**
	 * @var \Arm\T3sixshop\Library\Pdf\Fpdf;
	 */
	public static $pdf;
	...
	public static function init($orientation='P') {
		/*
		$fpdfPath = \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::siteRelPath('t3sixshop') . 'Classes/Library/Pdf/Fpdf.php';
		require_once "$fpdfPath";
		*/
		self::$pdf = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('Arm\\T3sixshop\\Library\\Pdf\\Fpdf');
		self::$pdf->SetAutoPageBreak(true, 8);
		self::$pdf->AliasNbPages();
		self::$pdf->SetAuthor('ARM Technologies');
	...
	public static function generatePDF($filename,$download=TRUE,$path=NULL)	{
		...
		if ($download == TRUE) {
			...
			if ($filename != '' && $path != '') {
				$file = $path.'/'.$filename;
				if (self::$pdf->Output($filename,'F') === false) {
